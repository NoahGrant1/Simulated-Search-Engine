...
class AvlTree
	{
	 private: 
	    ...
	    NODE<TYPE> *_update(/* add parameters*/);
	 ...
	 public:
	    ...
	    bool  AVL_Retrieve (TYPE& dataOut, KTYPE key);
	    void  AVL_Traverse (void (*process)(TYPE  dataProc)); //in-order
	    void AVL_Update(KTYPE key, TYPE &dataIn);
...
template <class TYPE, class KTYPE> //inputs- type = data , ktype = int
bool AvlTree<TYPE, KTYPE>  ::  AVL_Retrieve  (TYPE& dataOut, KTYPE key)
{
//	Local Definitions
	NODE<TYPE> *node;
...
//updates a value in the AVL tree( inputs frequency, key)
template <class TYPE, class KTYPE>
void AvlTree<TYPE,KTYPE>::AVL_Update(KTYPE key, TYPE &dataIn)
{
	//Local Definitions
		NODE<TYPE> *node;
